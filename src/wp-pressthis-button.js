...
document.getElementById("reblog-this-button").addEventListener("click", function() {
	...
	document.getElementById('reblog-this-blog-url').onkeydown = function(e){
	   if(e.keyCode == 13){
	     submitForm();
	   }
	};
	...
	function addEventListeners() {
		clickedContinueHandler 	= document.getElementById("reblog-this-continue"    ).addEventListener("click", clickedContinue);
		hideModalHandler 		= document.getElementById("reblog-this-close-modal" ).addEventListener("click", hideModal);
		submitFormHandler 		= document.getElementById("reblog-this-submit"      ).addEventListener("click", submitForm);
	}

	function removeEventListeners() {
		document.getElementById("reblog-this-continue"    ).removeEventListener("click", clickedContinueHandler);
		document.getElementById("reblog-this-close-modal" ).removeEventListener("click", hideModalHandler);
		document.getElementById("reblog-this-submit"      ).removeEventListener("click", submitFormHandler);
...
function createModal() {
	if (!document.getElementById("reblog-this")) {
		...
		contents	 += '<h3 id="reblog-this-url-prompt">Your Wordpress Site URL</h3>';
		contents	 += '<input type="text" id="reblog-this-blog-url" value="" placeholder="e.g. https://favorites.aribadernatal.com" size="40"/>';
